...
function Frameworks () {
    const renderFrameworks = frontEndFrameworks.map( ( e, i ) => <li key={i + new Date()}>{e}</li> ); // Change this line
    return (
        <div>
...
class MyComponent extends React.Component {
    ...
    render () {
        ...
        const renderOnline = usersOnline.map( ( e, i ) => ( <li key={i + new Date()}>{e.username}</li> ) ); // Change this line
        return (
            <div>
